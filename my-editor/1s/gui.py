...
class Ui_MainWindow(object):
    ...
    def setUi(self, MainWindow):
        ...
        ui.listWidget.addItems([yokais[y["type"]] for y in yokailist])
        ui.itemList.addItems([items[i["type"]] for i in itemlist])
        ui.equipmentList.addItems([equipments[e["type"]] for e in equipmentlist])
        ui.importantList.addItems([importants[i["type"]] for i in importantlist])
...
def updateYokai():
    ...
    yokailist[index]["type"] = reverse_yokais[indexs[ui.yokaiComboBox.currentIndex()+1].lower()]
    ui.listWidget.item(index).setText(yokais[yokailist[index]["type"]])
...
def setCurrentYokai():
    ...
    if index != previousItem: #then set all the data
        item.item(index).setText(yokais[yokailist[index]["type"]])
        ui.yokaiComboBox.setCurrentIndex(indexs.index(yokais[yokailist[index]["type"]])-1)
...
def newItemClicked():
    ...
    if itemName != "":
        ...
        item.addItems([items[itemlist[-1]["type"]]])
...
def currentItemClicked():
    ...
    if index != -1:
        if itemName == "":
            ...
            item.item(index).setText(items[itemlist[index]["type"]])
...
def newEquipmentClicked():
    ...
    if equipmentName != "":
        ...
        item.addItems([equipments[equipmentlist[-1]["type"]]])
...
def currentEquipmentClicked():
    ...
    if index != -1:
        if equipmentName == "":
            ...
            item.item(index).setText(equipments[equipmentlist[index]["type"]])
...
def newImportantClicked():
    ...
    if importantName != "":
        ...
        item.addItems([importants[importantlist[-1]["type"]]])
...
def currentImportantClicked():
    ...
    if index != -1:
        if importantName == "":
            ...
            item.item(index).setText(importants[importantlist[index]["type"]])
